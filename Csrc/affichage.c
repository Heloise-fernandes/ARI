...
void AfficherResultatHTMLCOULEUR(KEY *res, int len , char* pathURLS,char *req, char* final) 
{ 
  //Charger liste 
  ...
  { 
	fprintf(efe, "<tr>\n");    
    fprintf(efe, "<td>N°%d(%f) : %s</td>\n", i,res[i].d,liste[res[i].k]);
    fprintf(efe, "<td>N°%d(%f) : %s</td>\n", i+1,res[i+1].d,liste[res[i+1].k]);
    fprintf(efe, "</tr>\n");
...
void AfficherResultatHTMLCLUSTER(KEY *res, int len , char* pathURLS,char *req, char* final) 
{ 
  //Charger liste 
  char **liste = readList(pathURLS, &len); 
